...
from utils import RunningAverage, set_logger, Params
from model import *
...
parser.add_argument('--augm', type=str, default="False")
...
if args.augm == 'True':
    ...
    from data_loader import fetch_dataloader_no_augm as fetch_dataloader
...
def loss_fn_kd(outputs, labels, teacher_outputs, epoch, params):
    ...
    new_teacher_outputs = torch.empty(teacher_outputs.shape, device= teacher_outputs.device)
    for target in labels:

        same_target_indices = (labels == target)
        mean_tea_out = torch.mean(teacher_outputs[same_target_indices], dim=0)
        new_teacher_outputs[same_target_indices] = mean_tea_out


    KD_loss = (1. - alpha) * nn.CrossEntropyLoss()(outputs[0], labels) + \
    alpha * nn.KLDivLoss(reduction='batchmean')(F.log_softmax(outputs[0]/T, dim=1),
                     F.softmax(new_teacher_outputs/T, dim=1))* (alpha * T * T) 
...
def evaluate_self(model, loss_fn, data_loader, params):
    ...
    with torch.no_grad():
        ...
        print('student model:', params.model_name, params.teacher_resume, '\n')
...
if __name__ == "__main__":
    ...
    try:
        ...
        teacher_model.load_state_dict(remove_module(checkpoint['state_dict']))
    ...
    # # and the corresponding FC layer output where KD is utilized for skeptical student.
